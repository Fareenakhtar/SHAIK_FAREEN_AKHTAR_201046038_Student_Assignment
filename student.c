...
Program program_seats(){
	Program pg;
	printf("\nENTER BDA SEATS:\n");
	scanf("%d",&pg.bda_seats);
	assert(pg.bda_seats<=50);             //assert failed if seats of BDA is more than 50
	printf("\nENTER ML SEATS:\n");
	scanf("%d",&pg.ml_seats);
	assert(pg.ml_seats<=25);    //assert failed if seats of ML is more than 25
	printf("\nENTER ES SEATS:\n");
	scanf("%d",&pg.es_seats);
	assert(pg.es_seats<=25);    //assert failed if seats of ES is more than 25
	return pg;
}
...
int8_t declaration(){
	...
	printf("THERE IS TOTAL 100 SEATS*\n\n\n\n");
